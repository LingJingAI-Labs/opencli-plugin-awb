...
export const AUTH_PATH = path.join(os.homedir(), '.opencli', 'awb-auth.json');
export const AUTH_COMPAT_PATH = path.join(os.homedir(), '.animeworkbench_auth.json');
export const STATE_PATH = path.join(os.homedir(), '.opencli', 'awb-state.json');
...
export async function loadAuth() {
  const [primary, compat] = await Promise.all([
    ...
    readJson(AUTH_COMPAT_PATH, null),
  ]);
...
export async function saveAuth(auth) {
  const current = mergeAuthRecords(
    ...
    await readJson(AUTH_COMPAT_PATH, null),
  );
  const next = {
    ...current,
    ...auth,
  ...
  await Promise.all([
    writeJsonSecure(AUTH_PATH, next),
    writeJsonSecure(AUTH_COMPAT_PATH, toCompatAuthShape(next)),
  ]);
  return next;
}
...
export async function clearAuth() {
  await Promise.all([
    fs.unlink(AUTH_PATH).catch(() => {}),
    fs.unlink(AUTH_COMPAT_PATH).catch(() => {}),
  ]);
}
