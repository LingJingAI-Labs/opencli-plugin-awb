...
用生图模型先把人物、场景和剧情动作整理成一张 **4 宫格 / 9 宫格分镜指挥图**，再把这张图作为视频参考输入，配合人物主体 / 人设图 / 场景图，让参考生视频模型更容易按预期做镜头切换。

这条链路适合短剧自动化：先用 Banana Pro / Nano Banana / GPT Image 2 这类强图像模型做“视觉分镜板”，再交给 Seedance 2.0、可灵 3.0-Omni、Grok、Veo、Vidu、PixVerse 等参考生视频模型生成视频。

## 1. 何时使用

- 用户要“短剧分镜”“镜头切换可控”“按 4 个 / 9 个镜头走”
- 已有人物形象图 / 主体、场景图，希望视频模型按固定构图和节奏切镜
...
把分镜图作为 `分镜指挥图` 传给视频模型，同时继续传人物主体 / 人设图和场景图。prompt 里要明确：**学习分镜构图和切换顺序，不要把宫格边框、编号、字幕画进最终视频**。
...
- **视频生成**：效果优先 Seedance 2.0；预算优先 Seedance 2.0 Fast / Grok；可灵 3.0-Omni、Veo、Vidu、PixVerse 也可以走参考图路线，但先用 `model-options` 确认 `multi_param` / 参考图能力。
- **控音色**：继续用 `--refAudioFiles` / `--refAudioUrls` 绑定到同名人物主体或人物图；分镜指挥图只负责镜头，不负责音色。
...
- **人物一致性仍靠主体 / 人设图**：分镜图负责镜头节奏，不替代 `--refSubjects` 或人物形象图；场景也建议继续单独传。
