...
function getAwbCommandGroup(commandName) {
  const groups = {
    ...
    '素材与任务': new Set(['upload-files', 'subject-upload', 'subject-publish', 'subject-publish-batch', 'subject-status', 'subject-group-update', 'tasks', 'task-wait', 'task-records', 'task-record-poll', 'task-duration-stats', 'seedance-subtitle-remove', 'seedance-subtitle-status', 'aihubmix-video-status', 'aihubmix-video-download']),
  };
